...
*/
function getArtistByIndex(anArray, anIndex) {
  return `the artist at index ${anIndex} is ${anArray[anIndex].name}`;
}
...
function get20s(anArray){
  ...
  for (let i = 0; i < anArray.length; i++) {
    if (((anArray[i].years).slice(0,4)) >= 1900 && (anArray[i].years).slice(7) <= 2000) {
...
/*
let experiment = get20s(artists);

console.log(`experiment: ${experiment}`);

let birthDate = (artists[0].years).slice(0,4);
let death = (artists[0].years).slice(7);
...
*/
function removeArtist(anArray, indexNumToRemove) {
  anArray.splice(indexNumToRemove, 1);
...
let newArtist = ({
  ...
  "bio": `Shawn's love of all things tech began when he was just 11 years old, playing around with dos on Windows 3.1. His journey into web design was ignited at 39, when he joined Lambda. The rest, is history!`
});

function addArtist(anArray, aNewArtist){
  anArray.push(aNewArtist);
  return anArray;
}
